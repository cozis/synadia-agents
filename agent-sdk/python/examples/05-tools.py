...
async def chat(
    messages: list[dict[str, Any]], headers: dict[str, str] | None = None
) -> dict[str, Any]:
...
async def main() -> None:
    ...
    async def handler(envelope: Envelope, stream: PromptStream) -> None:
        ...
        # Round 1 — does the model want a tool? (non-streamed, for clean
        # tool_calls). Every outbound model request carries the thread's trace
        # headers so an observing proxy can correlate it.
        decision = await chat(messages, stream.trace_headers())
        ...
        async for token in chat_stream(messages, stream.trace_headers()):
