...
- Opt-in tracing through `tracing: "on"` / `NATS_TRACING=on` (also in the
  ...
  for a prompt that carries none, and publishes two signed `served` records
  per prompt on `TRACE.edges`, bound to `openclaw:<session id>` — the
  OpenClaw session the message was filed under, read from the session store
  after dispatch — with the outcome. OpenClaw's own model calls carry no
  thread id (a channel plugin cannot add provider headers); the served pair
  is what joins them to the thread. Requires `senderIdentity: "signed"`;
  without it nothing is published, the gateway warns at startup and the
  records owed count as dropped on the heartbeat.
