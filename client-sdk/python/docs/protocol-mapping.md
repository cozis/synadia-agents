...
| `verify_sender_header(header, arrival, payload, mode=…)` / `verify_sender(msg, mode)` (`VerifySender`) | Parse → (`live`: `ts` window) → `sub` acceptance by equality or, with `account_token_position`, by token removal after the token cross-check → (`live`: nonce lookup via `nonce_seen`) → ed25519. Cheap checks run before the signature; the outcome (`401`) is the spec's. `stored` skips freshness and uses the stored subject. Raises `SenderVerificationError` (`.code`). A `VerifiedSender` does not mean the nonce was recorded — the receiver records after every check passed. | Verification, JetStream and stored messages |
| `AgentInfo.identity` / `id_sig_verified` | `user_nkey` + `account` from the service metadata; `id_sig` verified over `AGENT-ID-V1\n{user}\n{account}\n{agent}\n{owner}\n{prompt_subject}\n` against the instance's own prompt endpoint subject — one ed25519 verification per discovered record with `id_sig`. | Registration |
| `Agents.resolve_sender(id)` (`ResolveSender`) | `$SRV.INFO.agents` scatter-gather indexed by verified `(account, user_nkey)`; unverified instances dropped; `None` when nobody claims the key; index cached for `resolve_ttl_s` (default 10 s). Identifies; never authorizes. | Reverse lookup |
