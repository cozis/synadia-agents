...
class MuxInbox:
    ...
    def mint_token(self) -> str:
        """Generate (but do not register) a fresh per-stream token, so a
        stream's reply subject — and anything derived from it, e.g. the
        observability thread id — is known before :meth:`register` and
        the publish."""
