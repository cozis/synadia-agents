...
- **Observability tracing (opt-in).** Pass `trace` to `Agents` to give every
  prompt a trace identity and write the interaction tree to the fabric's
  `TRACE` stream. Design:
  ...
  - `Agent.prompt()` mints a 128-bit `thread_id`, inherits `root_id` (and the
    ...
    so a node is visible even if the callee never runs. The prompt envelope
    carries only `thread_id` + `root_id`; the parent never transits the child.
  ...
  - Delivery is best-effort by contract and acked in practice: a bounded
    ...
    background with `Nats-Msg-Id` = `record_id`, exponential backoff on
    failure, and a flush on `Agents.close()`. `prompt()` only enqueues, so
    tracing never slows a caller. Tune via `trace.delivery`.
  - `traceHeaders()` returns `X-Synadia-Thread-ID` / `X-Synadia-Root-ID` for
  ...
  - `trace: { edgeSubject: null }` is propagate-only: mint and forward lineage,
    publish nothing. Without an identity signer nothing is published either
    (consumers ignore unsigned records) and the SDK warns once.
  - New exports: `TraceOptions`, `ActiveTrace`, `activeTrace`,
    `bindActiveTrace`, `inheritedTraceOptions`, `activeTurnCount`,
    `traceHeaders`, `formatTraceHeaders`, `buildEdgeRecord`, `EdgePublisher`,
    and the `HEADER_*` / `DEFAULT_EDGE_*` constants.

### Changed

- `RequestEnvelope` gains optional `threadId` / `rootId` (wire: `thread_id`,
  `root_id`). `decodeEnvelope` now **preserves** them instead of dropping them,
  and rejects a lone field or a malformed id as a §5.6 protocol error (a `400`
  on the agent side). Untraced prompts are byte-identical to protocol 0.3.
...
- **Sender identity (the sender-identity extension).** Every `prompt` /
  ...
  with a signer, an ed25519 signature bound to the subject, the payload,
  ...
  - `agents.signSender(subject, payload, { sub? })`,
    `agents.publishSigned(...)` (sets `Nats-Msg-Id` to the nonce),
    `agents.requestSigned(...)` (single reply) — signed publishes for any
    subject, JetStream included.
  - `Agent.status({ subject?, sub?, timeoutMs? })` — the §8.7 status
    probe, header attached; returns a `HeartbeatPayload`.
  ...
  - Discovery: `EndpointInfo.minSenderTrust`, `AgentInfo.supportsSenderIdentity`,
    `AgentInfo.identity` (`user_nkey` + `account`), `AgentInfo.idSigVerified`
    (`AGENT-ID-V1` over the prompt subject) — mirrored on `Agent`.
  - The shared codec, used by the host package too: `AgentId`
    (`newAgentId` / `parseAgentId` / `agentIdAccount` / `agentIdUser`),
    `parseSenderHeader` / `serializeSenderHeader` / `signSenderHeader` /
    `verifySenderHeader(header, arrivalSubject, payload, { mode: "live" | "stored", accountTokenPosition? })`,
    `SenderInfo = VerifiedSender | ClaimedSender` (a claim has no `id`),
    `formatSender` (always renders the trust class), `signAgentId` /
    `verifyAgentId`, `maxSenderHeaderBytes` / `expectedSenderHeaderBytes`.
  - `verifySender(msg, "live" | "stored", opts?)` — the spec-shaped
    `VerifySender` over a structural `{ subject, data, headers? }`
    (`Msg`, `ServiceMsg`, `JsMsg`; types `VerifiableMsg`,
    `VerifySenderMsgOptions`); `undefined` for no header / unknown `v`.
  - Reverse lookup (`ResolveSender`): `agents.resolveSender(id)` with
    `new Agents({ resolveTtlMs })` (default 10 000), `SenderResolver`
    (`resolve` / `invalidate` / `ttlMs`), the uncached `resolveSender(nc, id)`,
    `DEFAULT_RESOLVE_TTL_MS`, `normalizeResolveTtlMs` — `$SRV.INFO.agents`
    indexed by `AgentInfo.identity`, instances with a failing `id_sig`
    dropped, first verified instance returned, concurrent callers share
    one enumeration. `enumerateAgentInfos` (internal) now backs both
    `discover()` and the resolver.
  - Operator-attested mode in the codec (spec Appendix A):
    `VerifySenderOptions.operatorAttested` (+ `headers`, the message
    headers the stamp is read from), `NATS_REQUEST_INFO_HEADER`,
    `parseRequestInfo` / `readRequestInfo`, `RequestInfoStamp`.
    `VerifiedSender.accountAttested` is now set — under that mode only —
    when the stamp's `acc` (or the `accountTokenPosition` cross-check)
    agrees with the signed account; a disagreeing or unparseable stamp
    is a `401`; claims are never cross-checked.
  - `examples/_run-client-probe.ts`: `--signed`, `NATS_NKEY_SEED_FILE` /
    `NATS_CREDS` (authenticate the connection; with `--signed` also sign),
    a leading `{"type":"identity","id":…}` line under `--signed` (not
    counted in `done.chunks`), `name` / `code` on the stderr error line.
    Without `--signed` the probe sends no `Agent-Sender` at all.
  - `test-fixtures/identity/operator/`: an operator-mode fixture (T5 —
    113-char IDs, creds files, a `share: true` cross-account import)
    generated by `@nats-io/jwt` in its own package.
  - Errors: `IdentityError` (base), `NoIdentityError`,
    `IdentityUnavailableError`, `IdentityMismatchError`,
    `InvalidAgentIdError`, `MalformedSenderHeaderError`,
    `SenderSignatureRequiredError`, `SenderVerificationError`.
  - `readContextFile(selector)` — the reading half of
    `loadContextOptions`, so a signer can be built from a `nats` CLI
    context without building connection options.
  - `parseProtocolVersion` / `compareProtocolVersion` /
    `InvalidProtocolVersionError` are now exported from the package root
    (`InvalidProtocolVersionError` extends `NatsAgentError`).
  - New runtime dependency `@nats-io/nkeys` (ed25519 / key decoding).
  - Known-answer vectors for both signed inputs are generated by this
    package (`scripts/generate-identity-vectors.ts`) into
    `test-fixtures/identity/sender-vectors.json` and `id-sig-vectors.json`.
...
- **`prompt()` counts the `Agent-Sender` header against `max_payload`**
  ...
  `PayloadTooLargeError`, with `headerBytes` set), and the exact size is
  re-checked once the identity is known (rejected promise). The bound is
  conservative by up to ~100 bytes (64-byte account allowance, 64-char
  nonce) — a request that only fits without the allowance is refused.
- **`prompt()` may now reject** (not throw) with `NoIdentityError` /
  `IdentityUnavailableError` on a `min_sender_trust: signed` endpoint and
  with `IdentityMismatchError` whenever a signer is configured; it throws
