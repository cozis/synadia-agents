...
| Observability lineage (extension)    | `RequestEnvelope.threadId` / `rootId` (wire `thread_id` / `root_id`), sent together or not at all, each 32 lowercase hex; a lone field or a bad shape is a `ProtocolError` (`400`). Tolerated by 0.3 peers per §5.6; absent entirely when tracing is off. See [`observability.md`](https://github.com/synadia-ai/synadia-agent-fabric-docs/blob/master/docs/observability.md). | §5.6 |
...
The extension is additive to protocol `0.3`. The rows below map the SDK to
the extension's wire concepts without changing the core protocol.
...
| `agents.selfId()` (`SelfID()`)                                                                        | `$SYS.REQ.USER.INFO` request (2 s). A configured signer's user and JWT account are compared with the live answer. Memoised per connection and identity-source fingerprint; reconnect clears every source; failures retry after 30 s.                                                                                                                                                                                                                                                                                                                                    | When the server reports no identity         |
| `Agent-Sender` on `prompt` / `status`                                                                 | Omitted `identity` → no lookup/header; explicit `{}` → unsigned claim; `sendUnsignedClaim: false` → no automatic lookup/header; signer → live-bound signed header. Fresh NUID nonce; `ts` as `YYYY-MM-DDTHH:MM:SSZ`; canonical single-line JSON in the order `v, account, user, name?, sub?, ts?, nonce?, sig?`.                                                                                                                                                                                                                                                        | The `Agent-Sender` header, Wire form        |
...
| `verifySender(msg, mode)` / `verifySenderHeader(header, arrival, payload, { mode })` (`VerifySender`) | Parse → (`live`: `ts` window) → `sub` acceptance by equality or, with `accountTokenPosition`, by token removal after the token cross-check → (operator-attested cross-check) → (`live`: nonce lookup) → ed25519. Cheap checks run before the signature; the outcome (`401`) is the spec's. `stored` skips freshness and uses the stored subject (`msg.subject` of a `JsMsg`). Throws `SenderVerificationError` (`.code`).                                                                                                                                               | Verification, JetStream and stored messages |
| `operatorAttested` (`VerifySenderOptions`; host option)                                               | Off by default — a deployment promise (closed endpoint) the SDK cannot verify. On: a verified header's `account` / `user` must equal a present `Nats-Request-Info` `acc` / `user`; a present but unparseable stamp (or two of them) is refused; an absent stamp is compared to nothing (`401` on any disagreement, before the nonce lookup). Agreement on `acc` — or the `accountTokenPosition` cross-check — sets `VerifiedSender.accountAttested` (`formatSender` → `(verified)`). Unsigned claims are never cross-checked. `Nats-Request-Info` is read nowhere else. | Appendix A                                  |
| `agents.resolveSender(id)` / `SenderResolver` (`ResolveSender`)                                       | `$SRV.INFO.agents` enumerated and indexed by `AgentInfo.identity`; instances whose `id_sig` fails are dropped; the first verified instance for the key is returned, `undefined` otherwise. The index is cached for `resolveTtlMs` (default 10 s); concurrent callers share one enumeration. The host binds the same lookup into `VerifiedSender.resolve()`. Account-local; identifies, never authorizes.                                                                                                                                                                | Reverse lookup: from agent ID to agent      |
...
account allowance behind the size bound; under operator-attested mode a
`Nats-Request-Info` that is present but not parseable as the server's stamp is
refused like a disagreement (the spec defines only disagreement); the
`accountTokenPosition` cross-check counts as attestation only under
`operatorAttested` (the spec's "inserted token is a server stamp only on a
closed endpoint").
