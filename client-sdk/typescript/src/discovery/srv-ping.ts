...
export async function discoverAgents(
  ...
  return infos
    ...
    .map(
      (info) =>
        new Agent(nc, info, defaultInactivityTimeoutMs, closeSignal, identity, trace, logger),
    );
}
...
export async function lookupAgentInstance(
  ...
  if (!info) return null;
  return new Agent(nc, info, defaultInactivityTimeoutMs, closeSignal, identity, trace, logger);
}
