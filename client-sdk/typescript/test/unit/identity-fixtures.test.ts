...
//   - every seed in `keys.json` derives its listed public key;
//   - `agent-id-fixtures.json` is well-formed and its "real NKEY" claims
//     hold under the nkeys prefix + CRC check.
//
// No server needed. The host package carries the same file minus the
// `@nats-io/nkeys` checks (it has no direct nkeys dependency).
...
describe("test-fixtures/identity — keys.json", () => {
  it("lists the five throwaway users; each seed derives its public key", async () => {
    ...
    for (const [name, { public: pub, seed }] of Object.entries(keys.users)) {
      ...
      expect(fromSeed(new TextEncoder().encode(seed)).getPublicKey(), name).toBe(pub);
      fromPublic(pub); // prefix + CRC — throws on a corrupted key
    }
...
describe("test-fixtures/identity — agent-id-fixtures.json", () => {
  it("carries the spec's ten parse rows first, then the extra rows; valid rows split cleanly", async () => {
    ...
    for (const row of f.parse) {
      if (row.valid) {
        ...
        fromPublic(row.user ?? "");
        if (ACCOUNT_KEY.test(row.account ?? "")) fromPublic(row.account ?? "");
  ...
  it("its bad-CRC row keeps the NKEY shape but fails the nkeys check", async () => {
    const f = await loadJson<AgentIdFixtures>("agent-id-fixtures.json");
    const bad = f.parse.find((r) => r.note.includes("CRC"));
    expect(bad).toBeDefined();
    const account = bad?.input.split(".")[0] ?? "";
    expect(account).toMatch(ACCOUNT_KEY);
    expect(() => fromPublic(account)).toThrow(/checksum/);
  });
