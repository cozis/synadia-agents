...
| `trace-atp.conf`              | Observability: FABRIC (JetStream) exports `TRACE.*.edges` with `account_token_position: 2`; APP imports its own slice with `to: TRACE.edges`. The tenant can write but not read.        |
